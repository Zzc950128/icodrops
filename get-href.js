		...
		.end(function(err, res) {
			...
			for(let i = 0; i <= ico.length / 2; i++) {
				...
				let data = {
					...
					kyc: _this.find(".categ_one").length == 0 ? '' : replaceText(_this.find(".categ_one").attr('title')),
					whitelist: _this.find(".categ_three").length != 0 ? replaceText(_this.find(".categ_three").attr('title')) : _this.find("#linkwhitelist").length != 0 ? replaceText(_this.find("#linkwhitelist").attr('title')) : '',
					market: _this.find("#t_tikcer").length != 0 ? replaceText(_this.find("#t_tikcer li").text()) : ''
				}
				if(item.id == 0) {
